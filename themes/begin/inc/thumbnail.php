...
function zm_sites_thumbnail() {
	...
	if ( get_post_meta($post->ID, 'thumbnail', true) ) {
		...
		$n = count($strResult[1]);
		if($n > 0) {
			echo '<div class="load"><a href="'.$sites_img_link.'" target="_blank" rel="external nofollow"><img src="' . get_template_directory_uri() . '/img/loading.png" data-original="'.get_template_directory_uri().'/prune.php?src='.$strResult[1][0].'&w='.zm_get_option('img_w').'&h='.zm_get_option('img_h').'&a='.zm_get_option('crop_top').'&zc=1" alt="'.$post->post_title .'" /></a></div>';
		}
	}
}
